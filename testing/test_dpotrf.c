...
ratio(mkl/(blkintl+conv)), error(intl)\n");
    ...
    for (int N = 2; N < 33; N++){
        ...
        for (int testid = 0; testid < nbtest; testid++){
            ...
            memcpy_aptp2intl(arrayA, Aref, N, batch_count);
            clearcache();
            gettime();
            timediff = time;
        ...
        double error_intl =  get_error(Asol, Ap2p, N, N, batch_count);
        ...
        // Copute forward error
        double error_intl_conv =  get_error(Asol, Ap2p, N, N, batch_count);
        //Free work
        hbw_free(work);
        ...
        for (int BLOCK_SIZE = 8; BLOCK_SIZE <= MAX_BLOCK_SIZE; BLOCK_SIZE +=8) {
            ...
            for (int testid = 0; testid < nbtest; testid++){
                ...
                memcpy_aptp2blkintl(arrayAblk, Aref, N, BLOCK_SIZE, batch_count);
                clearcache();
                gettime();
            ...
            memcpy_ablkintl2ptp(Ap2p, arrayAblk, N, BLOCK_SIZE, batch_count);
            ...
            // Compute error
            error_blkintl =  get_error(Ap2p, Asol, N, N, batch_count);
        ...
        error_blkintl_conv =  get_error(Ap2p, Asol, N, N, batch_count);
